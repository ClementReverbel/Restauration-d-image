...
def calcul_SNR_chemin(chemin_image_base, chemin_image_bruitee) :
...
#Calcul du SNR entre une image de référence et une image bruitée
def calcul_SNR(image_base, image_bruitee) :
    #on recupere les images sous forme de matrice
    image_original = img_as_float(image_base)
    image_bruitee = img_as_float(image_bruitee)
    #on calcule la puissance du signal et du bruit
    p_signal = np.sum(image_bruitee ** 2)
    p_bruit = np.sum((image_bruitee - image_original) ** 2)
    #on gère le cas où le bruit est nul (même image)
...
if __name__ == '__main__':
    # Exemple d'utilisation — protégé pour qu'il ne s'exécute pas à l'import
    print(calcul_SNR_chemin("./images_reference/image_reference1.png","./images_reference/image1_bruitee_snr_10.8656.png"))